#!/usr/bin/env bash
# Build a Python-3.7 based precise-engine artifact using Docker buildx.
# Default target: ARMv7. Optional: arm64 via TARGET_ARCH=arm64.
# Output:
...
case "$TARGET_ARCH" in
  armv7)
    ...
    BASE_IMAGE="arm32v7/python:3.7-buster"
    OUT_DIR="$ROOT_DIR/artifacts/precise-engine-armv7"
    BUILDER_TAG="precise-armv7-builder:local"
    ARCH_GLOB="armv7l"
    ;;
  arm64|aarch64)
    DOCKER_PLATFORM="linux/arm64/v8"
    BASE_IMAGE="arm64v8/python:3.7-buster"
    OUT_DIR="$ROOT_DIR/artifacts/precise-engine-arm64"
    BUILDER_TAG="precise-arm64-builder:local"
    ARCH_GLOB="aarch64"
    ;;
  *)
    echo "ERROR: unsupported TARGET_ARCH '$TARGET_ARCH' (expected armv7 or arm64)"
    exit 1
    ;;
esac

WORK_DIR="$ROOT_DIR/.tmp-precise-build"
PRECISE_REPO="https://github.com/MycroftAI/mycroft-precise"
PRECISE_REF="v0.3.0"

mkdir -p "$OUT_DIR"

echo "[1/5] Checking for script changes (cache validity)..."
SCRIPT_HASH_FILE="$OUT_DIR/.build_script.sha256"
CURRENT_HASH="$(sha256sum "$0" | awk '{print $1}')"
if [ -f "$SCRIPT_HASH_FILE" ]; then
  CACHED_HASH="$(cat "$SCRIPT_HASH_FILE")"
  if [ "$CURRENT_HASH" = "$CACHED_HASH" ]; then
    echo "  ℹ️  Build script unchanged - Docker layer cache should help"
  else
    echo "  ℹ️  Build script changed - forcing full rebuild (Docker cache will be invalidated)"
    rm -rf "$WORK_DIR"
  fi
else
  echo "  ℹ️  First build - creating fresh work directory"
  rm -rf "$WORK_DIR"
fi
mkdir -p "$WORK_DIR"
echo "$CURRENT_HASH" > "$SCRIPT_HASH_FILE"

echo "[1.5/5] Checking Docker buildx support..."
if ! docker buildx version >/dev/null 2>&1; then
  echo "ERROR: docker buildx is required."
  exit 1
fi

echo "[1.5/5] Ensuring binfmt handlers for cross-arch builds..."
if ! docker run --privileged --rm tonistiigi/binfmt --install arm,arm64 >/dev/null 2>&1; then
  echo "WARNING: failed to install binfmt handlers automatically; cross-build may fail if QEMU isn't configured"
fi

echo "[2/5] Using stable Dockerfile (generates once per architecture)..."
DOCKERFILE_PATH="$OUT_DIR/.Dockerfile.${TARGET_ARCH}"
if [ ! -f "$DOCKERFILE_PATH" ]; then
  echo "  Creating Dockerfile for $TARGET_ARCH at $DOCKERFILE_PATH"
  cat > "$DOCKERFILE_PATH" <<DOCKERFILE
FROM $BASE_IMAGE

ENV DEBIAN_FRONTEND=noninteractive
# Buster is archived; point apt to archive mirrors.
...
WORKDIR /opt
DOCKERFILE
  echo "  Cached for future builds"
else
  echo "  Using cached Dockerfile from $DOCKERFILE_PATH"
fi

cat > "$WORK_DIR/build_inside.sh" <<'BUILD_SCRIPT'
#!/usr/bin/env bash
set -euo pipefail
...
echo "[3/5] Building $TARGET_ARCH builder image..."
docker buildx build \
...
# Mount pip cache to speed up rebuilds
PIP_CACHE_DIR="$HOME/.cache/precise-build-pip"
mkdir -p "$PIP_CACHE_DIR"
docker run --rm \
  -e PRECISE_REPO="$PRECISE_REPO" \
  -e PRECISE_REF="$PRECISE_REF" \
  -e OUT_DIR=/out \
  -e TARGET_ARCH="$TARGET_ARCH" \
  -e ARCH_GLOB="$ARCH_GLOB" \
  -v "$OUT_DIR":/out \
  -v "$WORK_DIR/build_inside.sh":/opt/build_inside.sh:ro \
  -v "$PIP_CACHE_DIR":/root/.cache/pip \
  "$BUILDER_TAG" \
  bash /opt/build_inside.sh

echo "[5/5] Validating bundle for missing imports..."
if [ -f "$ROOT_DIR/validate_precise_imports.py" ]; then
  if python3 "$ROOT_DIR/validate_precise_imports.py" "$OUT_DIR/precise-engine.tar.gz"; then
    echo ""
    echo "✅ Build complete and validated: $OUT_DIR/precise-engine.tar.gz"
    echo "Next: ./deploy_precise_engine_to_pi.sh pi $OUT_DIR/precise-engine.tar.gz"
  else
    echo ""
    echo "⚠️  Build complete but validation found missing imports!"
    echo "Review the validation output above before deploying."
    echo "Artifact: $OUT_DIR/precise-engine.tar.gz"
    exit 1
  fi
else
  echo "Warning: validate_precise_imports.py not found, skipping validation"
  echo "Build complete: $OUT_DIR/precise-engine.tar.gz"
  echo "Next: ./deploy_precise_engine_to_pi.sh pi $OUT_DIR/precise-engine.tar.gz"
fi
